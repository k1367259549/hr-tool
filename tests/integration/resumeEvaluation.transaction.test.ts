...
maybeDescribe("ResumeEvaluationResult transaction integration", () => {
  ...
  afterAll(async () => {
    ...
    const resumeIds = [resumeId, duplicateResumeId, staleResumeId].filter(
  ...
  it("creates evaluation atomically with CREATED event", async () => {
    const evaluation = await resumeEvaluationResultService.createEvaluation({
      evaluatedBy: "招聘官 A",
      jobProfileId,
      resumeId,
      templateVersionId
  ...
  it("updates draft and increments revision", async () => {
    ...
    const updated = await resumeEvaluationResultService.updateDraftEvaluation(id, {
      criterionResults: [
        {
  ...
  it("rejects update with stale revision", async () => {
    ...
    await expect(
      resumeEvaluationResultService.updateDraftEvaluation(id, {
        expectedRevision: 0,
  ...
  it("marks evaluation as REVIEWED", async () => {
    ...
    const reviewed = await resumeEvaluationResultService.reviewEvaluation(id, {
      actor: "招聘官 A",
      expectedRevision: 1
    });
  ...
  it("reopens REVIEWED evaluation to DRAFT", async () => {
    ...
    const reopened = await resumeEvaluationResultService.reopenEvaluation(id, {
      actor: "招聘官 A",
  ...
  it("rejects stale reopen after a complete reopen-edit-review cycle", async () => {
    ...
    const updated = await resumeEvaluationResultService.updateDraftEvaluation(evaluation.id, {
      criterionResults: [
        {
          assessment: "SUPPORTED",
          criterionKey: "backend-api",
    ...
    const firstReviewed = await resumeEvaluationResultService.reviewEvaluation(evaluation.id, {
      actor: "招聘官 A",
      expectedRevision: updated.revision
    });
    const staleExpectedRevision = firstReviewed.revision;

    const reopened = await resumeEvaluationResultService.reopenEvaluation(evaluation.id, {
      actor: "招聘官 A",
      expectedRevision: staleExpectedRevision,
      note: "补充评估证据"
    });

    const edited = await resumeEvaluationResultService.updateDraftEvaluation(evaluation.id, {
      criterionResults: [
        {
    ...
    const secondReviewed = await resumeEvaluationResultService.reviewEvaluation(evaluation.id, {
      actor: "招聘官 A",
      expectedRevision: edited.revision
    });

    await expect(
      resumeEvaluationResultService.reopenEvaluation(evaluation.id, {
        actor: "旧客户端",
        expectedRevision: staleExpectedRevision,
        note: "旧客户端重复重新开放"
      })
