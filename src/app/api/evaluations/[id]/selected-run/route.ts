...
import {
  parseResumeEvaluationSelectedRunPayload,
...
export async function PATCH(
  ...
  try {
    ...
    const input = parseResumeEvaluationSelectedRunPayload(body);
    const evaluation = await resumeEvaluationResultService.selectRunForReview(
      id,
      input.selectedRunId
    );
    ...
    return handleSelectedRunApiError(error);
  }
}

function handleSelectedRunApiError(error: unknown): Response {
