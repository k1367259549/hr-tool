...
export function FeishuShell({ children }: FeishuShellProps): JSX.Element {
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
      <aside className="min-w-0 max-w-full lg:sticky lg:top-6 lg:self-start">
        <div className="rounded-md border border-slate-200 bg-white shadow-sm">
          ...
          <nav className="flex min-w-0 max-w-full gap-2 overflow-x-auto p-3 lg:flex-col lg:overflow-visible" aria-label="飞书 V2 导航">
            {feishuModules.map((module) => (
              <Link
                key={module.href}
                href={module.href}
                className={`flex min-w-fit shrink-0 items-center gap-3 whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-slate-100 hover:text-slate-950 ${
